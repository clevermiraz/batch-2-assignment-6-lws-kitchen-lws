...
export const getRecommenedRecipe = (categoryId) => {
    if (!categoryId) {
        return null; // Return null if categoryId is not provided
    }

    // Filter recipes by category ID and sort by average_rating in descending order
    const recipes = recipesData
        .filter((recipe) => recipe?.category_id === categoryId)
        .sort((a, b) => b?.rating?.average_rating - a?.rating?.average_rating)
        .slice(0, 4); // Take the top 4 recipes

    return recipes.length ? recipes : null; // Return null if no recipes are found
};
